...
- A synchronization problem between the `DeviceFinder` and `VirtualCdj` would cause Beat Link to likely pick a device number that was already in use if you told it to self-assign and use a real device number. It’s unclear how long this problem has present, but it was serious.
