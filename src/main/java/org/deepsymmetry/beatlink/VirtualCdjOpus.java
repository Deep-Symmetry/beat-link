...
public class VirtualCdjOpus extends LifecycleParticipant {
    private static final Logger logger = LoggerFactory.getLogger(VirtualCdjOpus.class);
     ...
     * @throws IllegalStateException if the {@code VirtualCdjOpus} is not active
     */
    @API(status = API.Status.EXPERIMENTAL)
    ...
    public synchronized byte getDeviceNumber() {
        return virtualCdjOpusKeepAliveBytes[DEVICE_NUMBER_OFFSET];
    }
    ...
    public synchronized void setDeviceNumber(byte number) {
        ...
        virtualCdjOpusKeepAliveBytes[DEVICE_NUMBER_OFFSET] = number;
    }
     ...
     */
    private static final byte[] virtualCdjOpusKeepAliveBytes = {
     ...
     */
    @API(status = API.Status.EXPERIMENTAL)
    public void sendVirtualCdjOpusAnnouncement() {
        if (isRunning()) {
            DatagramPacket announcement = new DatagramPacket(virtualCdjOpusKeepAliveBytes, virtualCdjOpusKeepAliveBytes.length,
     ...
     */
    private boolean createVirtualCdjOpus() throws Exception {
        ...
        System.arraycopy(getMatchingInterfaces().get(0).getHardwareAddress(),
                0, virtualCdjOpusKeepAliveBytes, MAC_ADDRESS_OFFSET, 6);
        System.arraycopy(matchedAddress.getAddress().getAddress(),
                0, virtualCdjOpusKeepAliveBytes, 44, 4);
        ...
        }, "beat-link VirtualCdjOpus announcement/updates sender");
    ...
    private Thread createStatusReceiver() {
        ...
        }, "beat-link VirtualCdjOpus status receiver");
    ...
    private void sendAnnouncements() {
        try {
            sendVirtualCdjOpusAnnouncement();
    ...
    private final LifecycleListener deviceFinderLifecycleListener = new LifecycleListener() {
        ...
        public void started(LifecycleParticipant sender) {
            logger.debug("VirtualCdjOpus doesn't have anything to do when the DeviceFinder starts");
        }

        @Override
        public void stopped(LifecycleParticipant sender) {
            if (isRunning()) {
                logger.info("VirtualCdjOpus stopping because DeviceFinder has stopped.");
                stop();
            }
        }
    };
     ...
     *
     * @return true if we found DJ Link devices and were able to create the {@code VirtualCdjOpus}, or it was already running.
     ...
     */
    private static final VirtualCdjOpus ourInstance = new VirtualCdjOpus();
     ...
     */
    @API(status = API.Status.EXPERIMENTAL)
    public static VirtualCdjOpus getInstance() {
        return ourInstance;
    }
     ...
     */
    private VirtualCdjOpus() {
    }

    @Override
    public String toString() {
        StringBuilder sb = new StringBuilder();
        sb.append("VirtualCdjOpus[number:").append(getDeviceNumber()).append(", name:").append(getDeviceName());
